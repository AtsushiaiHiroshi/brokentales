...
const packageRoot = rootIndex >= 0 ? path.resolve(args[rootIndex + 1]) : path.resolve(__dirname, "..");
const requested = new Set(args.filter((arg, index) => (
  arg !== "--active-only"
  && arg !== "--root"
  && index !== rootIndex + 1
)));
...
async function readManifest(root) {
  for (const filename of ["system.json", "module.json"]) {
    const manifestPath = path.join(root, filename);
    if (await exists(manifestPath)) return JSON.parse(await fs.readFile(manifestPath, "utf8"));
  }
  throw new Error(`No system.json or module.json found in ${root}`);
}
...
for (const pack of selectedPacks) {
  try {
    const results = await syncPack(packageRoot, pack, timestamp);
